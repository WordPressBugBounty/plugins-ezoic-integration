...
class Ezoic_AdTester_Sidebar_Inserter extends Ezoic_AdTester_Inserter
{
	...
	public function insert()
	{
		...
		// Log sidebar insertion details
		$inserted_count = count($widget_options);
		Ezoic_Integration_Logger::console_debug(
			"Inserted {$inserted_count} ad widgets into sidebar '{$this->config->sidebar_id}' for page type '{$this->page_type}'",
			'Sidebar Ads'
		);

		// Log each inserted position
		foreach ($widget_options as $widget_id => $options) {
			if (isset($options['embed_code']) && preg_match('/id="ezoic-pub-ad-placeholder-(\d+)"/', $options['embed_code'], $matches)) {
				$position_id = $matches[1];
				Ezoic_Integration_Logger::track_insertion($position_id);
				Ezoic_Integration_Logger::console_debug(
					"Sidebar widget {$widget_id} contains position ID {$position_id}",
					'Sidebar Ads',
					'log',
					$position_id
				);
